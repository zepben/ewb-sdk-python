...
def print_network_hierarchy(client: SyncNetworkConsumerClient):
    ...
    for region in hierarchy.geographical_regions:
        current_location = hierarchy.geographical_regions[region]
        print(f"- {current_location.name} [{current_location.mrid}]")
        for sub_region in current_location.sub_geographical_regions:
