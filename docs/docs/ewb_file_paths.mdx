...
The Energy Workbench Server(EWB) uses a defined directory structure and naming convention for locating the various databases it is requested to load from within its "data path".
`EwbDataFilePaths` is a class for working with the file locations used by EWB to load databases from.
...
To generate file paths for the various databases used by EWB. Note: `EwbDataFilePaths` will return the expected path for the provided `database_type` and `date` regardless
