...
def get_with_base_voltage(service: NetworkService, client: SyncNetworkConsumerClient, mrid: str):
    equipment = client.get_identifiable(mrid).result
    if not equipment:
        return
    ...
    if mrids:
        client.get_identifiable(mrids[0])
```
