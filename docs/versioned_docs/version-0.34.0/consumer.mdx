...
with connect_with_password(client_id="some_client_id",username="user@email.com",password="password1",conf_address="https://ewb.zepben.com/ewb/auth",host="ewb.zepben.com",rpc_port=443,secure=True) as channel:
    client = SyncNetworkConsumerClient(channel)
    # ...
