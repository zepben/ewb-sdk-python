...
from zepben.ewb import NetworkService, AcLineSegment, Breaker
...
def single_branch_common_lines_network(sw_is_open: bool) -> NetworkService:
    ...
    # AcLineSegment1
    acls1 = AcLineSegment(mrid="acls1", length=1.0, per_length_impedance=plsi1)
    network.add(acls1)
    acls1_t = create_terminal(network, acls1)

    # AcLineSegment2
    acls2 = AcLineSegment(mrid="acls2", length=2.0, per_length_impedance=plsi1)
    network.add(acls2)
    ...
    # AcLineSegment3
    acls3 = AcLineSegment(mrid="acls3", length=3.0, per_length_impedance=plsi1)
    network.add(acls3)
    ...
    # AcLineSegment4
    acls4 = AcLineSegment(mrid="acls4", length=4.0, per_length_impedance=plsi2)
    network.add(acls4)
    ...
    # AcLineSegment5
    acls5 = AcLineSegment(mrid="acls5", length=5.0, per_length_impedance=plsi3)
    network.add(acls5)
