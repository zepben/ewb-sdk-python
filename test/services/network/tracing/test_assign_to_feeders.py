...
import pytest
from zepben.evolve import Equipment, TestNetworkBuilder, Feeder, BaseVoltage, Tracing, NetworkStateOperators
...
class TestAssignToFeeders:
    ...
    async def test_stops_at_lv_equipment(self):
        ...
        network_service = (TestNetworkBuilder()
                           .from_breaker(action=lambda ce: setattr(ce, "base_voltage", self.bv_hv))
                           .to_acls(action=lambda ce: setattr(ce, "base_voltage", self.bv_hv))
                           .to_acls(action=lambda ce: setattr(ce, "base_voltage", self.bv_lv))
                           .add_feeder("b0")
                           .network)
    ...
    async def test_includes_transformers(self):
        ...
        network_service = (TestNetworkBuilder()
                           .from_breaker(action=lambda ce: setattr(ce, "base_voltage", self.bv_hv))
                           .to_acls(action=lambda ce: setattr(ce, "base_voltage", self.bv_hv))
                           .to_power_transformer(end_actions=[lambda ce: setattr(ce, "base_voltage", self.bv_hv), lambda ce: setattr(ce, "base_voltage", self.bv_lv)])
                           .to_acls(action=lambda ce: setattr(ce, "base_voltage", self.bv_lv))
                           .add_feeder("b0")
                           .network)
