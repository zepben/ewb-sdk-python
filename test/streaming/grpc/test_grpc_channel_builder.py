...
def test_with_token_fetcher(
    mocked_ssl_channel_creds,
    mocked_md_call_creds,
    mocked_comp_channel_creds,
    mocked_secure_channel,
    mock_test_connection,
    mock_secure_sync_channel,
):
    GrpcChannelBuilder().for_address("hostname", 1234).make_secure().with_token_fetcher(
        ZepbenTokenFetcher("audience", "issuer_domain", "token_endpoint"),
    ).build()
...
def test_test_connection_uses_timeout(mock_checkConnection):
    ...
    mock_checkConnection.assert_has_calls(
        [
            call(CheckConnectionRequest(), timeout=12345, wait_for_ready=False),
            call(CheckConnectionRequest(), timeout=12345, wait_for_ready=False)
        ],
        any_order=False,
    )
