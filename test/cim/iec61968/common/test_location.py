...
from test.cim.common_testing_functions import extract_testing_args, verify
...
def test_location_constructor_default():
    ...
    verify_default_location(loc)
    verify_default_location(loc2)


def verify_default_location(loc):
...
@given(data())
def test_location_constructor_kwargs(data):
    verify(
        [Location, create_location],
        data, location_kwargs, verify_location_values
    )


def verify_location_values(loc, main_address, position_points, **kwargs):
...
def test_points_collection():
    # noinspection PyArgumentList
    verify_collection_ordered(Location,
