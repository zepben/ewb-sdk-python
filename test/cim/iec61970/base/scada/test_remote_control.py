...
def test_remote_control_constructor_default():
    ...
    verify_default_remote_control_constructor(rc)
    verify_default_remote_control_constructor(rc2)


def verify_default_remote_control_constructor(rc):
...
@given(data())
def test_remote_control_constructor_kwargs(data):
    verify(
        [RemoteControl, create_remote_control],
        data, remote_control_kwargs, verify_remote_control_values
    )


def verify_remote_control_values(rc, control, **kwargs):
