...
def test_diagram_constructor_default():
    d = Diagram()
    d2 = create_diagram()
    verify_default_diagram(d)
    verify_default_diagram(d2)


def verify_default_diagram(d):
...
@given(data())
def test_diagram_constructor_kwargs(data):
    verify(
        [Diagram, create_diagram],
        data, diagram_kwargs, verify_diagram_values
    )


def verify_diagram_values(d, diagram_style, orientation_kind, diagram_objects, **kwargs):
...
def test_diagram_objects_collection():
    verify_collection_unordered(Diagram,
