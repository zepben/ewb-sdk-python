...
def test_loop_constructor_default():
    loop = Loop()
    loop2 = create_loop()
    verify_default_loop_constructor(loop)
    verify_default_loop_constructor(loop2)


def verify_default_loop_constructor(loop):
...
def test_energy_source_phase_constructor_kwargs(data):
    verify(
        [Loop, create_loop],
        data, loop_kwargs, verify_loop_constructor_values
    )


def verify_loop_constructor_values(loop, circuits, substations, energizing_substations, **kwargs):
...
def test_circuits_collection():
    verify_collection_unordered(Loop,
...
def test_auto_two_way_connections_loop_constructor():
    ...
    assert s2.get_energized_loop(loop.mrid) == loop
