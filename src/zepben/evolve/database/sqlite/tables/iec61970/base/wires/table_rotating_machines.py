...
class TableRotatingMachines(TableRegulatingCondEq):
    ...
    def __init__(self):
        ...
        """
        Rated voltage in volts (nameplate data, Ur in IEC 60909-0). It is primarily used for short circuit data exchange
        according to IEC 60909. The attribute shall be a positive value.
        """

        self.p: Column = self._create_column("p", "NUMBER", Nullable.NULL)
        """
        Active power injection in watts. Load sign convention is used, i.e. positive sign means flow out from a node.
