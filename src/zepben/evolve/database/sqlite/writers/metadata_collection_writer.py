...
class MetadataCollectionWriter(object):
    ...
    def save(self, metadata_collection: MetadataCollection, writer: MetadataEntryWriter) -> bool:
        status = True

        for data_source in metadata_collection.data_sources:
            status = status and validate_save(data_source, writer.save, lambda e: logger.error(f"Failed to save DataSource '{data_source.source}': {e.message}"))
