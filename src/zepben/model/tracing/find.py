...
from zepben.model.tracing.util import normal_downstream_trace, current_downstream_trace
...
async def _trace(traversal: Traversal, from_: ConductingEquipment, to: ConductingEquipment):
    ...
    async def stop_contains(cetc):
        return cetc.equipment.mrid in extent_ids

    async def step(cetc, is_stopping):
        if is_stopping:
            path_found[0] = True

        equip.append(cetc.equipment)
...
async def _find(traversal_supplier: Callable[[SearchType, ...], Traversal], froms: List[ConductingEquipment], tos: List[ConductingEquipment]):
